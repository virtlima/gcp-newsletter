...
def get_newsletter_from_firestore(user_email):
    ...
    """
    try:
        doc_ref = db.collection('newsletters').document(user_email)
        doc = doc_ref.get()
        if doc.exists:
            return doc.to_dict()
        else:
            print(f"No newsletter found for {user_email}")
            return None
    except Exception as e:
